...
用法：
  python report_html.py --itin itinerary.json --out 报告.html
"""
import argparse
import html
import json
...
def build():
    ...
    src_u = "".join(f"<li><a href='{esc(u)}' target='_blank'>{esc(t)}</a>"
                    f"<span class='src-why'>{esc(w)}</span></li>"
                    for t, u, w in getattr(RM, "SOURCES_U", []))
...
/* ---------- Sources ---------- */
.sources{{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:26px;}}
@media(max-width:820px){{.sources{{grid-template-columns:1fr;}}}}
.sources ul{{margin:0;padding-left:0;list-style:none;}}
.src-c-wide{{margin:0;padding-left:0;list-style:none;columns:2;column-gap:32px;}}
@media(max-width:820px){{.src-c-wide{{columns:1;}}}}
...
.src-c-wide li{{font-size:12.5px;padding:7px 0;border-bottom:1px solid #f2eee6;
  break-inside:avoid;list-style:none;}}
.src-c-wide a{{color:var(--accent2);text-decoration:none;font-weight:500;}}
.src-c-wide a:hover{{text-decoration:underline;}}
.src-why{{display:block;color:var(--muted);font-size:11.5px;margin-top:1px;}}
.src-c-wide .src-why{{display:block;color:var(--muted);font-size:11.5px;margin-top:1px;}}
...
<h2>八、信源质量说明</h2>
<div class="card">
  <p style="font-size:14px">本次共检索 <strong>528 条</strong>结果（三片区 × 17 组查询词），经规则引擎自动分级：</p>
  <table class="tbl">
    <thead><tr><th>等级</th><th>数量</th><th>含义</th></tr></thead>
    <tbody>
      <tr><td><strong>A 权威</strong></td><td class="num">116 条</td>
        <td>新华网、新昌县政府、本地宝、高德、知乎精品攻略、12306</td></tr>
      <tr><td><strong>U 用户实拍</strong></td><td class="num">5 条</td>
        <td>小红书笔记 / B站实拍 / 大众点评——鲜活但需交叉验证</td></tr>
      <tr><td><strong>C 待核</strong></td><td class="num">393 条</td>
        <td>携程/同程/去哪儿/境外OTA，票价与时效需现场核对</td></tr>
      <tr><td><strong>D 噪声</strong></td><td class="num">14 条</td>
        <td>地域无关内容（境外活动表、港澳台活动页等），已排除</td></tr>
    </tbody>
  </table>
  <div class="alert warn" style="margin-top:20px">
    <span class="ic">⚠️</span>
    <div><strong>为什么默认没有小红书？</strong>
    实测：默认 10 组通用查询词检索 120 条，小红书域命中<strong>仅 1 条</strong>——
    anysearch 的索引不会主动返回小红书，不点名它几乎拿不到。本次已追加
    <code>site:xiaohongshu.com</code> 定向查询（408 条结果中命中 8 条），
    并在分级表中新增 <strong>U 级（用户实拍）</strong>。详见下方 U 级清单。</div>
  </div>
  <div class="sources" style="margin-top:24px">
    <div>
      <h4>U 级信源 · 用户实拍（本次真实检索到）</h4>
      <ul>{src_u}</ul>
    </div>
    <div>
      <h4>A 级信源清单（可点击复核）</h4>
  ...
  <div style="margin-top:20px">
    ...
    <ul class="src-c-wide">{src_c}</ul>
  </div>
  <div class="alert warn" style="margin-top:20px">
    <span class="ic">⚠️</span>
    <div><strong>票价核对提醒</strong>：新昌各景区票价在不同平台差异较大
    （大佛寺 66/70/80，十九峰 53/56/60），本报告取线上优惠价区间。
    <strong>以购票平台实际显示为准</strong>。U 级源只作交叉印证，
    <strong>绝不单独作为票价/开放时间/预约规则的依据</strong>。</div>
  </div>
</div>

<footer>
  报告由 trip-plan 规则引擎生成 · 多片区自驾扩展版<br>
  anysearch 实时检索 528 条 · 生成时间 {date.today().isoformat()}
</footer>
</div>
</body>
</html>"""
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", help="输出 .html 路径（默认 ./<title>.html）")
    args = ap.parse_args()

    if args.itin:
        RM.load_itin(args.itin)
    h = build()
    out = args.out or os.path.join(os.getcwd(), args.title + ".html")
    os.makedirs(os.path.dirname(os.path.abspath(out)), exist_ok=True)
    open(out, "w", encoding="utf-8").write(h)
    print(f"HTML → {out}  ({len(h)} 字符)")
